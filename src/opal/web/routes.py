...
from opal.web.context import (
    get_base_context,
    get_current_user,
    require_admin_web,
    templates,
)
from opal.web.execution_context import build_execution_context, set_bar_step
...
def executions_detail(
    ...
    context = build_execution_context(request, db, instance)
    set_bar_step(context, None)
...
def users_new(request: Request, db: DbSession) -> HTMLResponse:
    ...
    redirect = require_admin_web(request, db)
    if redirect:
        return redirect
...
def users_issue_claim_link(request: Request, db: DbSession, user_id: int) -> HTMLResponse:
    ...
    """
    if require_admin_web(request, db) is not None:
...
def settings_auth_form(request: Request, db: DbSession) -> HTMLResponse | RedirectResponse:
    ...
    if redirect := require_admin_web(request, db):
        return redirect
